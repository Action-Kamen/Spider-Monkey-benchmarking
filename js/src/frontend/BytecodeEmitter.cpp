...
// static bool sDumpBytecode = false;

// #ifdef ENABLE_BYTECODE_DUMP
BytecodeEmitter::~BytecodeEmitter() {
    // Only dump for the outermost (parent==nullptr) script:
    // if (!parent && true) {
      DumpBytecodeToFile();
  // }
}

void BytecodeEmitter::DumpBytecodeToFile() {
  const auto& section = bytecodeSection_;  // alias
  const js::frontend::BytecodeVector& code = section.code();  // all bytes
  size_t length = code.length();  // total number of bytes

  // Open a hardcoded output file (overwrite mode):
  FILE* out = std::fopen("spidermonkey_bytecode_dump.txt", "a");
  if (!out) {
      return;  // failed to open file
  }

  // Iterate through the bytecode and print offsets and opcodes:
  size_t pc = 0;
  while (pc < length) {
      JSOp op = JSOp(code[pc]);           // opcode at this byte
      unsigned opLen = js::GetOpLength(op); // length of this instruction in bytes

      // Print offset and opcode name (if available):
      // cast the JSOp to an integer type so you can index the array:
      auto idx = static_cast<uint8_t>(op);
      // std::fprintf(out, "%04zu: %s\n", pc, js::CodeNameTable[idx]);
      std::fprintf(out, "%s\n",js::CodeNameTable[idx]);

      // Print each immediate byte after the opcode:
      // for (unsigned i = 1; i < opLen && pc + i < length; i++) {
      //     std::fprintf(out, " %u", code[pc + i]);
      // }
      // std::fprintf(out, "\n");
